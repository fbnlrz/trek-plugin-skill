...
| `validate [dir]` | — | Manifest + layout checks (same manifest rules as the install loader). Fails on invalid `trek-plugin.json`, a missing or unsatisfiable **`trek` range** (required — TREK refuses to install a plugin without one), missing `README.md`, or missing `server/index.js`; warns if dir name ≠ id, README lacks a screenshot, scaffold placeholders remain, the name/description contain **emoji** (TREK strips them from host-rendered text), `requiredAddons` names an unknown addon, **`icon` is not a real lucide name** (TREK would silently fall back to `Blocks`, and registry CI rejects it — so this warning is your only local catch, since `preflight` does *not* replay that gate), **`trek` is `"*"`** (claims every TREK, past and future), or the client ships a raw `<select>` without the design kit. Since `pack` validates first, a missing README also fails `pack`. **Subset of CI** — CI additionally verifies release/sha256/README over the network. |
...
| `preflight [dir] --repo <o/n> --tag <vX.Y.Z> [--all] [--entry <file.json>] [--zip] [--commit] [--sign]` | network | Runs the **full registry CI locally**: tag→commit, manifest parity (incl. `trek`/`operatorEgress`/`requiredAddons`/`pluginDependencies`), the `trek`-range gate (present, satisfiable, and matching the manifest), artifact sha256 + size, **author-signature shape + verification**, native scan, README gate. **Default checks only the newest version; `--all` checks every `versions[]`.** Green preflight ⇒ green CI **except** for CI-only gates: full JSON-schema validation (e.g. the 200-char description cap), homoglyph names, owner binding, the **icon-name gate** (an `icon` lucide doesn't have — `validate` warns about it, but preflight does not check it), and the **signing-downgrade guard** (it compares against the entry on the PR base, which only exists in CI). |
