...
| `description` | string | no | One-line store summary. **Keep it ≤ 200 chars:** `validate`/`pack` don't check length, but the registry entry schema caps it at 200 and CI rejects a longer one **after** you've cut the release (`buildEntry` copies it verbatim). Min 8 chars on the entry side. |
